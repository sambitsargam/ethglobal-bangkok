  ...
  return (
    <Card className="shadow-md transition-transform hover:scale-[1.01]">
      <CardHeader className="flex flex-row items-center justify-between py-6">
        <CardTitle className="text-lg font-bold text-white-800">Listed Pools</CardTitle>
      </CardHeader>
      <CardContent>
              ...
              {pools.slice(0, 1)?.length ? (
                pools.slice(0, 1).map((pool, index) => (
                  <TableRow
                    ...
                    <TableCell>
                      {formatUnits(pool.amount, pool.asset.decimals)}
                    </TableCell>
                    <TableCell>
                      {formatUnits(pool.apr, APR_DECIMALS)}%
                    </TableCell>
                    <TableCell>
                      {formatUnits(pool.ltv, LTV_DECIMALS)}%
                    </TableCell>
                    ...
                    <TableCell>
                      {formatUnits(pool.amount / BigInt(2), pool.asset.decimals)}{" "}
                      {pool.asset.symbol}
                    </TableCell>
                    <TableCell>
                      <Button
